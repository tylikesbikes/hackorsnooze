...
class StoryList {
   ...
   */
    ...
    const res = await axios.post('https://hack-or-snooze-v3.herokuapp.com/stories', {token:user.loginToken, story:newStory});
...
class User {
  ...
  async favorite(story) {
    if(this.isFavorite(story)===false) {
  ...
  async unFavorite(story) {
    ...
    if ($favoritesList.is(':hidden')===false) {
    putFavoritesOnPage();
    }
  ...
  async _addOrRemoveFavorite(action, story) {
    ...
    // if(method==='POST') {
    //   axios.post(`${BASE_URL}/users/${this.username}/favorites/${story.storyId}`, {token:currentUser.loginToken});
    // } else if (method==='DELETE') {
    //   axios.delete(`${BASE_URL}/users/${this.username}/favorites/${story.storyId}`, {params:{token:currentUser.loginToken}});
    // }
  }
